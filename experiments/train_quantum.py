"""Train the CV-Quixer quantum model.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the CV-Quixer model")
...
def main() -> None:
    ...
    assert config.model == "quantum", (
        f"Config model is '{config.model}', expected 'quantum'. "
        f"Use experiments/train_classical.py for the classical baseline."
    )
    ...
    print(f"Model: CV-Quixer | Parameters: {model.get_num_parameters():,}")
