"""Build (and launch) the positional-encoding ablation sweep.

Phase B of the PE ablation (see the project handoff). The block-1 CNN
hypernetwork injects a **2D sinusoidal positional encoding** into its conv
features before the gate-parameter projection. Phase A made that a configurable
``positional_encoding`` knob ({none, 1d, 2d}, default 2d, byte-identical). This
builder fans the **16 curated epoch-extension configs** (the ones in
``results/extended_runs_25ep.txt``, spanning all three quantum models) over the
``{none, 1d}`` variants into one fresh-from-scratch **32-run** sweep.

The **2d arm is deliberately NOT generated** — it is reused from the existing
``high_epoch_*`` runs and compared manually in a notebook (capped at epoch 10),
so this builder only emits ``none`` / ``1d``.

For each of the 16 source argv (read verbatim from the source sweep's
``sweep_manifest.json`` ``runs[].args``) × ``{none, 1d}`` it:

  * injects ``--positional-encoding <v>``,
  * rewrites ``--run-name`` to append ``__pe<v>`` (sweep.py's marker spelling),
  * repoints ``--runs-root`` at the new ``results/sweeps/pe_ablation_<ts>`` dir,
...
Runs are **ordered so each target-GPU group is a contiguous index range** (carry
the GPU map from ``results/extended_runs_25ep.txt``), so the two SLURM array
slices can be submitted separately over the one manifest. The manifest's
``runs[]`` schema is the one ``scripts/run_sweep.sh`` consumes unchanged.

Examples
--------
Inspect the 32-run plan only::

    uv run python experiments/build_pe_ablation.py --dry-run

Submit both GPU groups as SLURM array slices::

    uv run python experiments/build_pe_ablation.py --launch slurm
"""
...
# The two PE variants this sweep trains from scratch. 2d is reused (not regenerated).
PE_VARIANTS = ("none", "1d")
...
# headroom (quantum/shared ran 4.6-8.0 h). GPU_ORDER fixes the contiguous
# group layout in the manifest (a100 indices first, then h100, then h200).
...
def pe_run_name(original_run_name: str, pe: str, model: str = "quantum") -> str:
    """The PE-ablation dir name: ``<model>__<original>__pe<v>``.
    ...
    one ``pe_ablation`` dir would collide (and silently clobber) those two runs'
    directories without the prefix. ``__pe<v>`` keeps sweep.py's marker spelling.
    """
    return f"{model}__{original_run_name}__pe{pe}"
...
def resolve_source_runs(rows: list[dict]) -> list[dict]:
    ...
    Returns ``{run_name, gpu, args}`` with ``args`` read verbatim from the source
    ``sweep_manifest.json``. Raises ``KeyError`` if a selected run is not in its
    source manifest (a stale selection list — fail loud, never silently drop).
    """
...
def rewrite_run_args(
    args: list[str], pe: str, *, model: str, runs_root: str, target_epochs: int,
) -> list[str]:
    """Original argv rewritten for one fresh PE-ablation run (never resumed).

    Injects ``--positional-encoding``, rewrites ``--run-name`` to the
    ``<model>__…__pe<v>`` form (``pe_run_name``), repoints ``--runs-root``,
    ...
    if "--run-name" in out:
        ...
        out[i] = pe_run_name(out[i], pe, model)
    else:  # pragma: no cover - source argv always carries --run-name
        out.extend(["--run-name", pe_run_name("run", pe, model)])
    _set("--positional-encoding", pe)
...
def build_manifest(
    ...
    *,
    sweep_dir: Path,
    pe_variants: tuple[str, ...] = PE_VARIANTS,
    target_epochs: int = DEFAULT_EPOCHS,
) -> dict:
    """Fan the resolved source runs over ``pe_variants`` into a sweep manifest.
    ...
    ``index`` range; within a group, source order is preserved and the variants
    are emitted in ``pe_variants`` order. The returned ``slurm_groups`` maps each
    present GPU to its ``[lo, hi]`` inclusive index range.
    """
    ...
    for gpu in GPU_ORDER:
        ...
        for src in group:
            ...
            for pe in pe_variants:
                entries.append({
                    "index": len(entries),
                    "run_name": pe_run_name(src["run_name"], pe, model),
                    "source_run_name": src["run_name"],
                    "model": model,
                    "positional_encoding": pe,
                    "gpu": gpu,
                    "args": rewrite_run_args(
                        src["args"], pe, model=model, runs_root=runs_root,
    ...
    if dups:
        raise ValueError(
            f"duplicate run name(s) in the PE-ablation plan: {dups} — the "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build the positional-encoding ablation sweep: 16 curated "
        "configs × {none, 1d}, fresh from scratch (the 2d arm is reused, not "
        "regenerated).",
    ...
    parser.add_argument(
        ...
        help="parent dir for the new pe_ablation_<ts> sweep dir",
    )
    parser.add_argument(
        "--sweep-name", type=str, default="pe_ablation",
    ...
    print(f"PE ablation sweep: {manifest['sweep_name']}  ({manifest['n_runs']} runs)")
    print(f"  source runs:   {args.source_runs_file}  ({len(source_runs)} configs)")
    print(f"  pe variants:   {manifest['pe_variants']}")
