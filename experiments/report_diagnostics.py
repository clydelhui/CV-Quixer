...
def _diag_stages(diag: dict) -> list[tuple[str, str, str]]:
    ...
    (ADR-0002): stacked runs write block-prefixed keys (``block{b}_…`` per
...
def plot_training_curves(run: dict) -> None:
    ...
    # ADR-0002). Canonical runs log a zero stand-in every epoch, so skip when
...
def _collect_gate_param_grids(
    ...
    ADR-0002). Returns ``(key_grid, query_grid)``; the query grid is empty for
...
def sanity_checks(run: dict) -> None:
    ...
    # block{b}_/agg_ prefixes, ADR-0002).
    diag = run["diagnostics"]
    if diag is not None:
