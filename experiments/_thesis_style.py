...
class CaptionBook:
    ...
    def write(self, quiet: bool = False) -> Path | None:
        ...
        A partial render must not truncate a fuller caption file. Figures skip
        when their artefacts are absent — running on a laptop that holds only
        `history.json` produces 3 of 12 — and blindly overwriting would silently
        drop the other nine captions while their figures sat on disk beside it.
        When the existing file covers more figures, the partial output is
        diverted to a `.partial.tex` sibling and the good file left alone.
        """
        ...
        if path.is_file():
            existing = path.read_text().count("\\begin{figure}")
            if existing > len(self._entries):
                path = path.with_suffix(".partial.tex")
                warnings.warn(
                    f"{self.filename} already covers {existing} figures but this "
                    f"run rendered {len(self._entries)} — writing {path.name} "
                    "instead of truncating it. Re-render the full set (on the "
                    "cluster, where every artefact is present) to refresh it.",
