...
- **Manual mode** — set architecture knobs **directly** as grid axes (no
  ...
  `--coeff-ablation`, `--poly-mode`, plus the
  ...
  `__penone`/`__pe1d`/`__pe2d`, `__calcu`/`__calcu_poly`, `__qsvt`, …).
...
| `poly_mode` | `"standard"` | Polynomial construction mode (CONTEXT.md "Polynomial mode", ADR-0009): how each degree-`j` term of `P(M)=Σ_j c_j M^j` is built from the LCU `M`. `"standard"` (default) applies `M` `j` times — the literal power `M^j` (byte-identical to a pre-knob model; absent key reloads as `"standard"`). `"qsvt"` alternates `M` with its adjoint `M†` by application parity (`v_1=Mv_0`, `v_2=M†v_1`, …) so term `j` is `M†M M†…` (`j` factors, rightmost `M`): even terms `(M†M)^{j/2}=VΣ^jV†` stay in the input singular-vector space, odd terms `M(M†M)^{(j-1)/2}=WΣ^jV†` map input→output, and their `c_j`-weighted sum is the QSVT singular-value transform (parity branches superposed by plain vector addition). `M†` is the true adjoint of the *truncated* `M` (conjugate-transpose of the Fock gate matrices, NOT param-negation). Reuses the same `b_i`/`c_j`/`U_i` (structurally identical state_dict); only the forward computation changes. Coincides with `"standard"` at `poly_degree ≤ 1`. Lives in the shared head base → applies to all model variants. A valid manual sweep axis (`__qsvt` marker) |
...
(`none`/`1d`/`2d`), `--poly-mode` (`standard`/`qsvt`; ADR-0009).
...
| `debug/` | Always-on NaN forensics (`cv_quixer/utils/debug_nan.py`): `init_fingerprint.json` (per-param init checksums, written at the launch that created the run), `stream.npz` + `stream_meta.json` (per-batch per-head trunc / W-trunc, per-group grad norms `head{h}/{hypernet,lcu,poly,cvqnn}` + `decoder`, gate-param min-abs and exactly-0.0 counts per `{op}_{param}` type, per-head success-prob min/max — rewritten each epoch and at events), and one `nan_event_step{N}/` per new non-finite event (pre-step `snapshot.pt` with model/optimizer/batch/grads + `record.json` + `anomaly_report.txt` from a `detect_anomaly` replay naming the failing backward node, capped at 8 dumps). On-event policy: dump, keep training, abort at the end of the first epoch with no new event (`debug/aborted_nan.json` + `history["meta"]["nan_aborted"]`). **OOM forensics** (`cv_quixer/utils/debug_oom.py`): the train step is wrapped in `try/except torch.cuda.OutOfMemoryError`; on a catchable CUDA OOM it writes `debug/oom_event.json` (`cause`, `epoch`/`step`/`batch_index`, `error_message` from `str(exc)`, peak/allocated/reserved MB, device, and the grid `corner` = `num_heads`/`num_modes`/`cutoff_dim`/`poly_degree`/`num_seq2seq_blocks`), mirrors the same dict into `history["meta"]["oom_aborted"]`, prints an `[OOM abort]` marker to the tee'd `train.log`, then **re-raises** (ADR-0004: log, never swallow — the original CUDA traceback still reaches the SLURM `.err`). Scope is the catchable CUDA OOM only; host-RAM SIGKILL and wall-time SIGTERM are out of scope (no observed casualty died that way; a heartbeat/handler would be purely additive). Background: the 2026-06-12 grid-sweep OOM post-mortem (casualties had no `history.json`, dying at the epoch-table header on the 40 GB A100-40's heaviest Fock-sim corners). Background: the 2026-06 sweep NaN post-mortem — the beamsplitter/displacement gate builders *were* NaN-singular in gradient at exactly-zero params (fixed at the source; pinned by `tests/test_gate_gradients.py` + `tests/golden/`), and the post-selection renorm deliberately does **not** sanitise NaN: non-finite failures stay loud and this instrumentation owns detection (ADR-0004). |
